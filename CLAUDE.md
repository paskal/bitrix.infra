...
- **`docker compose` needs ALL profiles enabled or it errors `no such service: adminer`.** Optional services sit behind profiles (`certs`, `dbadmin`, `monitoring`, `hooks`, `ftp`); `nginx` `depends_on` `adminer` (profile `dbadmin`), so any bare `docker compose <cmd> nginx` fails to resolve the dependency. Always prefix with `COMPOSE_PROFILES=certs,dbadmin,monitoring,hooks,ftp` (compose 2.6.0 here is too old for the `--profile "*"` wildcard). E.g. recreate: `cd /web && COMPOSE_PROFILES=certs,dbadmin,monitoring,hooks,ftp docker compose up -d --force-recreate --no-deps nginx`.
