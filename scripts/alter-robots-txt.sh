...
# Also, block /montag/ and /projects/ for Tula as they're not present there.
...
for f in $robots_files; do
  ...
  robots_tula.favor-group.ru.txt)
    echo "Processing ${f}"
    # disallow blog, montag, and projects sections
...
echo "robots.txt files other than Moscow have /info/blog/ disallow entry now, and Tula disallows /montag/ and /projects/ also"
